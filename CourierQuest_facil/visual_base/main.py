...
def main():
    ...
    # Sprite para la IA CPU (tintado para diferenciarlo del jugador humano)
    img_cpu = cargar_imagen_robusta("mensajero.png", TAM_JUGADOR)
    try:
        img_cpu = img_cpu.copy()
        img_cpu.fill((80, 180, 255), special_flags=pygame.BLEND_RGBA_MULT)
    except Exception:
        # Si falla el tintado, simplemente se usa la misma imagen base
        pass
    ...
    def reset_partida():
        ...
        # Reiniciar también el estado de la IA CPU
        reset_cpu_state()
        primera_tardanza_fecha = None
        entregas = 0
    ...
    def actualizar_clima():
        ...
        if weather_system.transicion:
            ...
            if t >= 1.0:
                ...
                weather_system.intensidad_actual = (
                                                               1 - t) * weather_system.intensidad_inicio + t * weather_system.intensidad_objetivo
            return
    ...
    def update_cpu_facil(dt):
        ...
        if cpu_energia <= 0:
            ...
            if objetivo is not None:
                # Moverse hacia el objetivo (pickup o dropoff) con evitación sencilla de obstáculos
                tx, ty = objetivo
                ...
                if dx == 0 and dy == 0:
                    ...
                    # Priorizamos movernos primero en el eje más lejano (x o y)
                    dirs_preferidas = []
                    if abs(dx) >= abs(dy):
                        # primero horizontal, luego vertical
                        step_x = 1 if dx > 0 else -1
                        step_y = 1 if dy > 0 else -1 if dy != 0 else 0
                        dirs_preferidas.append((step_x, 0))
                        if step_y != 0:
                            dirs_preferidas.append((0, step_y))
                    else:
                    ...
                    # Direcciones de respaldo si las preferidas chocan
                    dirs_backup = [(1, 0), (-1, 0), (0, 1), (0, -1)]
                    # Evitar duplicados
                    dirs_candidatas = []
                    for d in dirs_preferidas + dirs_backup:
                        if d not in dirs_candidatas:
                            dirs_candidatas.append(d)
    ...
    while corriendo:
        ...
        if cpu_rect is not None:
            ...
            # En dificultad fácil, la CPU siempre se mueve más lento que el jugador
            cpu_vel = 0.6 * calcular_velocidad(cpu_peso_total, cpu_reputacion, cpu_energia,
        ...
        # expiración de pedidos de la CPU
        ...
        dibujar_hud(pantalla, font, entregas, dinero_ganado, reputacion,
                    ...
                    msg, energia, cpu_entregas, cpu_dinero, cpu_reputacion, cpu_energia)
