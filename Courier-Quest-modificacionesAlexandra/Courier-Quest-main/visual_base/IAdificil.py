...
class IADificil:
    ...
    """
    def dibujar_info_estado(self, pantalla):
        #Dibuja información de estado sobre la IA
        try:
            font = pygame.font.SysFont(None, 16)
            
            # Texto con información básica
            estado_texto = f"IA Difícil: ${self.dinero_ganado} | Ent: {self.entregas}"
            texto = font.render(estado_texto, True, (255, 100, 100))
            pantalla.blit(texto, (self.rect.x, self.rect.y - 35))
            
            # Estado actual
            estado = font.render(self.estado_actual, True, (255, 200, 100))
            pantalla.blit(estado, (self.rect.x, self.rect.y - 20))
            
            # Barra de energía
            barra_ancho = 40
            barra_alto = 4
            pygame.draw.rect(pantalla, (60, 60, 60), 
                            (self.rect.x, self.rect.y - 15, barra_ancho, barra_alto))
            energia_relleno = int((self.energia / 100) * barra_ancho)
            if energia_relleno > 0:
                color_energia = (0, 200, 0) if self.energia > 30 else (255, 200, 0)
                pygame.draw.rect(pantalla, color_energia, 
                               (self.rect.x, self.rect.y - 15, energia_relleno, barra_alto))
                
        except Exception as e:
            print(f"Error dibujando info IA: {e}")
    """
