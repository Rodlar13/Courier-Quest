...
def intentar_spawn_oferta(ofertas, ultimo_spawn, max_ofertas, intervalo_ms,
                          jugador_rect, img_cliente, obstaculos, pantalla):
    ahora = pygame.time.get_ticks()

    # Solo spawnear si hay espacio
    if len(ofertas) >= max_ofertas:
        return ultimo_spawn

    # Si ya pasó el tiempo de spawn
    if ahora - ultimo_spawn >= intervalo_ms:
        # Generar un nuevo pedido
        nuevo = nuevo_pedido(jugador_rect, img_cliente, obstaculos, pantalla)
        if nuevo:  # Asegurarse de que se haya creado
            ofertas.append(nuevo)
        return ahora  # actualizar el tiempo del último spawn
